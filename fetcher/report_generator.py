"""
日报生成器 - 生成午间速览和全天复盘
优化：批量获取数据，减少 API 调用次数，使用 D1 推送
"""
...
from yfinance_fetcher import get_stock_spot as get_us_stock_spot, get_market_status, get_us_indices_batch
...
class ReportGenerator:
    ...
    def save_and_push_report(self, content: str, report_type: str = 'daily'):
        """
        保存报告并推送到 D1
        替代原来的本地 SQLite 存储
        """
        ...
        try:
            ...
            result = self.push_client.push_report(
                report_date=date,
            ...
            print(f"✓ 报告已推送至 D1：{report_type} - {date}")
            return result
    ...
    def push_market_summary(self):
        ...
        try:
            result = self.push_client.push_market_summary(
                date=date,
                market='A股',
                up_count=market_summary.get('up', 0),
                down_count=market_summary.get('down', 0),
                flat_count=market_summary.get('flat', 0),
                limit_up=market_summary.get('limit_up', 0),
                limit_down=market_summary.get('limit_down', 0),
                metadata={
                    'up_ratio': market_summary.get('up_ratio', 0),
                    'total': market_summary.get('total', 0)
                }
            )
            print(f"✓ 市场概览已推送至 D1")
            return result
...
def main():
    ...
    parser = argparse.ArgumentParser(description='生成并推送财经日报')
    parser.add_argument('type', choices=['noon', 'daily', 'market'], help='报告类型')
    parser.add_argument('--date', help='指定日期 (YYYY-MM-DD)')
    parser.add_argument('--push-only', action='store_true', help='仅推送，不生成')
    ...
    if args.type == 'market':
        ...
        # 生成并推送午间速览
        print("生成午间速览...")
        content = generator.generate_noon_report(args.date)
        print(content)
        print("\n推送到 D1...")
        ...
        # 生成并推送全天复盘
        print("生成全天复盘...")
        content = generator.generate_daily_report(args.date)
        print(content)
        print("\n推送到 D1...")
